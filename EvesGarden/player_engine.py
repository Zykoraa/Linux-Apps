...
with warnings.catch_warnings():
    # pydub's last release predates Python 3.12, which started warning about
    # invalid escape sequences in string literals. Its utils module has four,
    # so importing it prints a dozen lines of SyntaxWarning about somebody
    # else's regexes -- the first thing anyone launching from a terminal
    # sees, and nothing they can act on.
...
def _quiet_alsa():
    """Silence the wall of ALSA and JACK errors PortAudio prints on startup.

    Opening PortAudio enumerates every ALSA device, and on a PipeWire system
    a dozen of the entries in /usr/share/alsa are plugins for hardware that
    is not there. Each one prints its complaint from C:

        ALSA lib pcm_oss.c:404:(_snd_pcm_oss_open) Cannot open device /dev/dsp
        Cannot connect to server socket err = No such file or directory
        jack server is not running or cannot be started

    None of it means anything -- playback works regardless -- but it is the
    first thing anyone launching from a terminal sees, and it reads like the
    app is broken. It is written to file descriptor 2 by the C library, so
    redirecting sys.stderr does nothing; the descriptor itself has to go.

    Only around the constructor, so a real error later still reaches the
    terminal.
    """
    try:
        saved = os.dup(2)
...
class PlayerEngine:
    ...
    def _ensure_ffmpeg():
        ...
        The Windows build hardcoded `bin\\ffmpeg.exe` because Windows has
        nowhere else to find one. Here the system copy is preferred and a
        bundled one is only used if it is actually present -- see
        ffmpeg_tools for why that is the right way round on Linux.
        """
    ...
    def load_stream(self, url, duration=0.0, title=None, ffmpeg=None):
        ...
        if not ffmpeg:
            self.last_error = ffmpeg_tools.MISSING_MESSAGE
            return False
