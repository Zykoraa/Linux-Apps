...
Two locations, checked in order:
  ...
  portable  -- an `EvesGarden.env` file sitting next to the app. Kept from
               the Windows build, where it made the zip self-contained, and
               still useful for running a checkout straight out of a folder.
  user      -- ~/.config/evesgarden/env, which is where an installed copy
               writes. The Linux install puts the app under
               ~/.local/share, which is not somewhere config belongs, so
               unlike on Windows this is the normal case rather than a
               fallback for a read-only Program Files.

The file has no leading dot. A dotfile inside ~/.config is hidden twice over
and invisible to `ls ~/.config/evesgarden`, which is where someone goes
looking when they want to edit or delete their keys.
...
def env_paths():
    ...
    return [
        ...
        user_path(),
        os.path.join(user_dir(), ".env"),         # the older dotted name
    ]
...
def save(client_id, client_secret, discord_id=None, redirect_uri=None):
    ...
    lines = [
        ...
        "# Keep this file next to gui.py to stay portable.",
    ...
    # The Windows build preferred writing beside the executable, because
    # that is what made a copied folder keep working. An installed Linux
    # copy lives under ~/.local/share, which is writable but is not where
    # configuration belongs -- so the config directory wins here, and the
    # portable file is only written to when one already exists.
    target = portable_path() if os.path.exists(portable_path()) else user_path()
