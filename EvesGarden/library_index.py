...
# Folders that are never a music library and are often huge. The Windows
# names stay: an indexed folder can be a mounted NTFS drive, which brings
...
_SKIP_DIRS = {"node_modules", "__pycache__", "$recycle.bin",
              ...
              # The Linux ones worth naming. Dot-directories are already
              # skipped by the walk, which covers ~/.cache, ~/.local and
              # the trash inside it -- so only the visible offenders are
              # listed: the kernel filesystems, which are not files, and
              # the two directories that are enormous and never music.
              "proc", "sys", "dev", "run", "steamapps",
              "lost+found", "snap"}
