...
def _log_dir():
    ...
    ~/.local/state is where the XDG spec puts a log: state that should
    survive a reboot but that nobody wants backed up or synced. The Windows
    build put these in %LOCALAPPDATA% alongside the settings and the index;
    keeping logs out of ~/.config means a cleaner or a dotfile sync never
    sees them.
    """
    return xdg_paths.ensure(xdg_paths.state_dir())


LOG_DIR = _log_dir()
if getattr(sys, "frozen", False):
    # Only hijack the streams in the frozen build; running from source
...
# The Windows build wrapped every subprocess.Popen to pass STARTF_USESHOWWINDOW,
# because each yt-dlp and ffmpeg call flashed a console window over whatever
# you were doing. Nothing on Linux opens a window to run a program, so the
...
# One folder per kind of state, which is what the XDG spec is for. The
# Windows build put settings, the index and the logs in one directory
# because %LOCALAPPDATA% is the only place it had; here, ~/.config is what
# gets backed up and ~/.cache is what a disk cleaner may delete, and the
# library index belongs in neither -- it is regenerable data, not settings.
LIBRARY_DIR = xdg_paths.library_dir()
CONFIG_DIR = xdg_paths.ensure(xdg_paths.config_dir())
DATA_DIR = xdg_paths.ensure(xdg_paths.data_dir())
SETTINGS_PATH = os.path.join(CONFIG_DIR, "settings.json")
INDEX_PATH = os.path.join(DATA_DIR, "library.db")
...
class App(ctk.CTk):
    def __init__(self):
        ...
        # Frameless, but still a window the compositor manages. See
        # linux_window: overrideredirect(True) on X11 would take the window
        # out of the taskbar, out of Alt-Tab and out of the tiling layout.
        self.wm = linux_window.LinuxWindow(self)
        self.wm.undecorate()
        self.wm.set_class()
        self._apply_window_icon()
        self.title("Eve's Garden")
    ...
    def _setup_desktop_media(self):
        """Publish the player on D-Bus, which is where media keys come from.

        The Windows build called RegisterHotKey for the four media keys --
        already an improvement on the pynput keyboard hook before it, which
        saw every keystroke typed in any application just to notice three.

        On Linux the keys are not the application's to claim. The compositor
        owns them and the user has bound them, so a player that grabbed
        XF86AudioPlay would be fighting every other player for it. What the
        desktop expects instead is MPRIS, and announcing the player there
        gets far more than the hotkeys did: media keys routed by the
        compositor to whichever player is active, `playerctl` and anything
        built on it, and the track, artwork and transport buttons in waybar,
        GNOME's panel, KDE's applet and the lock screen.
        """
        direct = {
        ...
        with_value = {
            ...
            "shuffle": self._mpris_shuffle,
            "loop": self._mpris_loop,
        }

        def dispatch(action, value=None):
            # Called on the D-Bus thread; nothing here may touch Tk directly.
            if action in direct:
                self._safe_after(0, direct[action])
            elif action in with_value:
                self._safe_after(0, with_value[action], value)

        self.mpris = mpris.MprisService(
            dispatch, position_provider=self.player.get_position)
        try:
            if not self.mpris.start():
                print("MPRIS unavailable: no session bus. Media keys and "
                      "panel controls will not reach this player.")
        except Exception as e:
    ...
    def _mpris_play(self):
        """MPRIS separates Play from Pause; the app has one toggle."""
    ...
    def _mpris_seek(self, offset):
        self._mpris_set_position(self.player.get_position() + (offset or 0.0))

    def _mpris_set_position(self, seconds):
        duration = self.player.get_duration()
        if duration <= 0:
            return
        seconds = max(0.0, min(float(seconds or 0.0), duration))
        self.player.set_progress(seconds / duration)
        try:
            self.progress_slider.set(seconds / duration)
        except Exception:
            pass
        # Panels only redraw their scrubber when told a jump happened.
        self.mpris.seeked(seconds)

    def _mpris_volume(self, value):
        value = max(0.0, min(1.0, float(value or 0.0)))
        self.volume_slider.set(value)
        self.on_volume(value)

    def _mpris_shuffle(self, wanted):
        if bool(wanted) != bool(self.shuffle):
            self.toggle_shuffle()

    def _mpris_loop(self, status):
        wanted = status in ("Track", "Playlist")
        if wanted != bool(self.repeat):
            self.toggle_repeat()

    def _raise_window(self):
        """Bring the window back, for MPRIS Raise() and the tray's Show."""
        try:
            self.deiconify()
            self.lift()
            self.wm.activate()
    ...
    def _restore_audio_controls(self):
        # `or` would be wrong here, and loudly so: a saved volume of 0.0 is
        # falsy, so quitting muted and reopening came back at full volume.
        saved = self.settings.get("volume")
        volume = float(saved if saved is not None else 1.0)
    ...
    def toggle_play_pause(self):
        if self.player.playing and not self.player.paused:
            self.player.pause()
            self.play_btn.set_glyph("play")
            self._publish_now_playing(playing=False)
        else:
            self.player.play()
            self.play_btn.set_glyph("pause")
            self._publish_now_playing(playing=True)
    ...
    def build_ui(self):
        ...
        self.theme_dropdown = ui_widgets.InWindowOptionMenu(
        ...
        self.view_menu = ui_widgets.InWindowOptionMenu(
            self.library_header, values=list(VIEWS),
            command=self.set_library_view, corner_radius=theme_ui.RADIUS_PILL,
        ...
        self.sort_dropdown = ui_widgets.InWindowOptionMenu(
        ...
        self.dedupe_btn = ctk.CTkButton(
            self.library_actions, text="Move ticked to Trash",
    ...
    MIN_W, MIN_H = 900, 620
    # How close to an edge of the work area counts as aiming at it. Only
    # used on a window manager that declines _NET_WM_MOVERESIZE; the ones
    # that take the drag bring their own edge threshold.
    SNAP_EDGE = 14

    def get_pos(self, event):
        """Start a drag -- by asking the window manager to take it over.

        The Windows build could not do this. It tried the equivalent trick
        there (ReleaseCapture, then SendMessage with WM_NCLBUTTONDOWN and
        HTCAPTION) and it killed the process on the first click of the title
        bar: SendMessage does not return until the move is over, Windows
        runs a modal loop meanwhile, and that loop dispatches messages back
        into Tk while ctypes still has the GIL released.

            Fatal Python error: PyEval_RestoreThread: the function must be
            called with the GIL held, but the GIL is released

        _NET_WM_MOVERESIZE has none of that failure mode, because it is not
        a call. It is a ClientMessage posted to the root window: it returns
        at once, Python is never inside it, and the window manager takes the
        pointer grab and runs the drag.

        Handing the drag over is also the only way to get behaviour that is
        actually right per desktop. The window manager already knows where
        the monitors are, where the panels are, what half-tiling means and
        whether this window is tiled at all -- under a tiling compositor,
        dragging a window is a swap with another tile, not a change of
        coordinates, and no amount of geometry() could express that.
        """
        self.xwin, self.ywin = event.x, event.y
        self._dragging = True
        self._drag_motions = 0
        self._wm_dragging = self.wm.begin_move(event.x_root, event.y_root)

    def move_window(self, event):
        """Move the window ourselves, if the window manager did not.

        When the window manager accepts the drag it takes the pointer grab,
        so motion events stop arriving here -- which makes a motion event
        that does arrive the signal that it declined. One straggler can
        already be in flight from before the handover, so it takes two
        before we conclude we are on our own and go back to moving the
        window by geometry, the way the Windows build always did.
        """
        if not getattr(self, "_dragging", False):
            return
        if getattr(self, "_wm_dragging", False):
            self._drag_motions += 1
            if self._drag_motions < 2:
                return
    ...
    def end_drag(self, event):
        ...
        Only reached on a window manager that declined the drag. The ones
        that took it did their own snapping, with their own edge threshold
        and their own idea of which monitor the pointer is on, and a second
        snap on top of that would fight it.
        """
        if not getattr(self, "_dragging", False):
            return
    ...
    def _work_area(self):
        """The desktop minus panels, read from the window manager.

        Not assumed: snapping to half the screen height puts the bottom of
        the window behind waybar, a GNOME dock or a KDE panel on every
        machine that has one. _NET_WORKAREA is what the window manager
        publishes for exactly this, though a compositor is entitled not to
        set it -- Hyprland advertises the hint and leaves the property
        empty -- so the whole screen remains the fallback.
        """
    ...
    def build_eq_overlay(self):
        ...
        self.preset_dropdown = ui_widgets.InWindowOptionMenu(self.eq_header, variable=self.preset_var, values=list(self.presets.keys()), command=self.on_eq_preset, corner_radius=10)
    ...
    def build_now_playing_overlay(self):
        ...
        self.viz_dropdown = ui_widgets.InWindowOptionMenu(
        ...
        self.viz_palette_dropdown = ui_widgets.InWindowOptionMenu(
    ...
    # The Windows build had a set_appwindow() here that rewrote the window's
    # style bits after creation: drop WS_EX_TOOLWINDOW to get a taskbar
    # button back, and add WS_THICKFRAME and the maximise box, because the
    # shell refuses to snap or tile a frameless WS_POPUP without them.
    #
    # None of that has an equivalent here, and none of it is needed. An X11
    # window that is not override-redirect is a normal managed window: it is
    # in the taskbar, in Alt-Tab and in the tiling layout already, and
    # removing the decorations with _MOTIF_WM_HINTS does not change that.
    # Snapping and tiling are the window manager's, not the window's.
    ...
    def _build_resize_grips(self):
        """Edge strips to resize by, since the window has no visible border.

        A window manager will still resize this window from its own border
        or keybinding -- undecorating it does not make it fixed-size the way
        overrideredirect did on Windows. The strips are for the mouse: with
        no drawn frame there is nothing obvious to grab, so these give the
        pointer somewhere to land and a resize cursor when it does.
        """
        ...
        # X11 cursor names. The Windows build used size_nw_se and size_ne_sw,
        # which are Win32-only spellings: Tk raises TclError: bad cursor spec
        # on them here, and the exception comes out of widget construction --
        # so the app did not merely show the wrong pointer, it failed to
        # build a window at all.
        cursors = {"e": "right_side", "w": "left_side", "s": "bottom_side",
        ...
        for edge, (size, place) in edges.items():
            ...
            grip.bind("<ButtonRelease-1>", lambda e, s=edge: self._resize_end(e, s))
            self._grips[edge] = grip

    def _resize_start(self, event, edge):
        """Hand the resize to the window manager, as with a move.

        Same reasoning as get_pos, and the same fallback: the grips keep
        working by geometry if the window manager will not take it.
        """
        self._resize_origin = (
        ...
        self._resize_motions = 0
        self._wm_resizing = self.wm.begin_resize(edge, event.x_root,
                                                 event.y_root)

    def _resize_drag(self, event, edge):
        if not getattr(self, "_resize_origin", None):
            return
        if getattr(self, "_wm_resizing", False):
            self._resize_motions += 1
            if self._resize_motions < 2:
                return
    ...
    def _resize_end(self, event, edge=None):
        self._resize_origin = None
        self._wm_resizing = False

    def toggle_maximize(self, event=None):
        """Maximise through the window manager, or fake it if it will not.

        _NET_WM_STATE is the real thing: it maximises to the work area, so
        the window stops above waybar or a KDE panel rather than under it,
        and it survives a monitor change. A tiling compositor may well
        ignore it -- a tiled window has no "maximised" to be -- which is the
        same answer every other application gets there, and the reason this
        keeps the geometry fallback for window managers that support
        neither.
        """
        if self.wm.toggle_maximize():
            return
    ...
    def minimize_to_tray(self):
        """Minimise -- to the tray if there is one, otherwise to the taskbar.

        On Windows there is always a tray, so withdrawing the window and
        putting an icon there was safe. On Linux there may be no tray at
        all: pystray needs an AppIndicator host or an XEmbed system tray,
        and a Wayland session running waybar without its tray module has
        neither. Withdrawing the window there would hide it with nothing
        left to click, and the only way back would be to kill the process.

        So the tray is attempted, and only if it actually comes up is the
        window withdrawn. Otherwise this iconifies: a normal minimise to the
        taskbar, which works on every desktop and is what the window being
        WM-managed rather than override-redirect buys.
        """
        if self._tray_icon is not None:
            self.withdraw()
            return

        icon = self._start_tray_icon()
        if icon is None:
            self.iconify()
            return
        self._tray_icon = icon
        self.tray_icon = icon      # kept for backwards compatibility
        self.withdraw()

    def _start_tray_icon(self):
        """Run a tray icon, or return None if this desktop has no tray.

        pystray raises from run() on its own thread when no tray host
        answers, so "did it work?" cannot be read from the return value --
        ...
        icon = pystray.Icon("EvesGarden", image, "Eve's Garden", menu)

        visible = threading.Event()

        def run():
            try:
                icon.run(setup=lambda _: visible.set())
            except Exception as e:
                print(f"No system tray available: {e}")
                visible.set()

        threading.Thread(target=run, daemon=True).start()
        visible.wait(timeout=2.0)
    ...
    def _sync_brand(self, _event=None):
        ...
        # every part of it is checked before any of it is measured -- every
        # part _place_brand goes on to touch, not just the five it used to
        # name. On Windows the missing ones never bit: nothing resizes a
        # window between its creation and the end of build_ui. A tiling
        # compositor does exactly that, the moment the window is mapped, so
        # here the first <Configure> arrives mid-build and reached
        # sort_dropdown before there was one:
        #
        #     AttributeError: '_tkinter.tkapp' object has no attribute
        #     'sort_dropdown'
        #
        # raised inside a Tk callback, which prints and carries on -- so the
        # app opened with a header that had never been measured.
        ...
        if None in (head, word, mark, tabs, actions):
            return
        for name in ("sort_dropdown", "new_pl_btn", "import_pl_btn",
                     "dedupe_btn", "repair_btn", "nav_dl_btn",
                     "nav_radio_btn", "view_menu",
                     "lib_search_entry", "brand"):
    ...
    def _sync_dedupe_button(self):
        ...
        if self.library_view == "Duplicates" and getattr(self, "library", None):
            ...
            button.configure(text=f"Move {marked} to Trash",
    ...
    def remove_duplicates(self):
        ...
        These go to the desktop's trash rather than being deleted outright,
        so a wrong call is undoable from the file manager -- see recycle,
        which implements the freedesktop trash spec every one of them reads.
        """
    ...
    def _finish_dedupe(self, removed, failed):
        note = f"Moved {removed} file{'' if removed == 1 else 's'} to the Trash"
        if failed:
    ...
    def show_track_menu(self, path, x, y):
        ...
        menu.add_separator()
        menu.add_command(label="Show in file manager",
    ...
    def _reveal(self, path):
        """Open the file manager on a track. See file_manager for how."""
        try:
            if file_manager.reveal(path):
                return
        except Exception as e:
            print(f"Could not open the file manager: {e}")
        self.library_status.configure(
            text="No file manager available to show that in")
    ...
    def _begin_track(self, file_path):
        ...
        try:
            ...
            row = None
        if row is None:
            # Not in the index -- which is the normal case for a file opened
            # from a file manager, since the .desktop entry offers this app
            # for any audio file and most of them will be outside the
            # library. The tags are in the file; without reading them the
            # bar, the full-screen view and the panel all show the filename
            # and nothing else.
            row = self._row_from_tags(file_path)
    ...
    def _row_from_tags(self, file_path):
        """A library-shaped row read straight from the file, or None."""
        try:
        ...
        if not tags:
            return None
        row = {"path": file_path}
        for key in ("title", "artist", "album", "albumartist", "year",
                    "track_no", "disc_no", "duration"):
            if tags.get(key):
                row[key] = tags[key]
        return row
    ...
    def _publish_now_playing(self, playing=None):
        """Tell everything outside the window what is playing.

        Two audiences with different needs. Discord wants a remote URL,
        because Rich Presence fetches the image itself and cannot be handed
        the artwork embedded in the file. MPRIS wants a local one -- a
        file:// URL the panel reads straight off disk -- so it shows
        artwork for a track Spotify has never heard of, and shows it with
        no network at all.
        """
        row = getattr(self, "_now_playing_row", None)
        if not row:
        ...
        self._push_mpris(row, title, playing)
    ...
    def _push_mpris(self, row, title, playing):
        service = getattr(self, "mpris", None)
        if service is None or not service.active:
            return

        # A new object path per track: panels key their state off it, and
        # reusing one makes every song look like the same song still going.
        path = row.get("path")
        if path != getattr(self, "_mpris_track_for", object()):
            self._mpris_track_for = path
            self._mpris_track_path = service.next_track_path()

        service.update(
            title=title,
            artist=row.get("artist") or "",
            album=row.get("album") or "",
            art_url=self._mpris_art_url(row),
            duration=self.player.get_duration(),
            playing=playing,
            track_path=getattr(self, "_mpris_track_path", None),
            can_next=bool(self.current_playlist),
            can_prev=bool(self.current_playlist),
            shuffle=bool(self.shuffle),
            loop="Playlist" if self.repeat else "None",
            volume=self.player.volume,
        )

    def _mpris_art_url(self, row):
        """Artwork for the panel: the cached cover file, else the remote one.

        Embedded APIC art cannot be sent over MPRIS -- the property is a URL,
        not bytes -- so it has to be a file already on disk. metadata.py
        already writes one out to cache the thumbnails this app draws, and
        that same file is what the panel reads.
        """
        path = row.get("path")
        if path:
            try:
                cached = audio_files.cover_file(path)
                if cached:
                    return mpris.file_url(cached)
            except Exception:
                pass
        return row.get("cover_url") or None
        ...
        def work():
            try:
                ...
                if (getattr(self, "_now_playing_row", {}) or {}).get("path") == file_path:
                    ...
                    self._safe_after(0, self._publish_now_playing)
            except Exception as e:
    ...
    def _load_and_play(self, file_path):
        if self.player.load_track(file_path):
            self.player.play()
            # Publish again now the length is known. The first push happens
            # in _begin_track, before the decode, when get_duration() is
            # still zero -- and a panel that is told a track has no length
            # draws no scrubber for it. The periodic re-push would fix it
            # eventually, but not for another fifteen seconds.
            self._safe_after(0, self._publish_now_playing, True)
...
def _paths_from_argv(argv):
    """Audio files named on the command line, as a desktop passes them.

    The .desktop file declares the audio MIME types this app can play, so
    "Open with Eve's Garden" in a file manager launches it with the file --
    and a file manager sends a file:// URI, not a path. Without this the
    argument was ignored and the app opened on an empty library, which is a
    worse answer than not offering to open the file at all.
    """
    ...
    for argument in argv:
        if argument.startswith("file://"):
            argument = unquote(urlparse(argument).path)
        elif "://" in argument:
            continue                      # a remote URL is not ours to play
        if os.path.isfile(argument):
            paths.append(os.path.abspath(argument))
    return paths
...
if __name__ == "__main__":
    ...
    opened = _paths_from_argv(sys.argv[1:])
    if opened:
        # After the window is up: play_from_library touches widgets that
        # build_ui has created but that have not been laid out yet.
        app.after(120, lambda: app.play_from_library(opened[0]))
