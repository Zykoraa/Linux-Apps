...
def main():
    ...
    frames = {s: build(DESIGN, s) for s in ICO_SIZES}

    # One PNG per size, laid out the way the hicolor icon theme wants them.
    # A .desktop file names an icon without an extension and the desktop
    # picks the size it needs, so a single 256px PNG is not enough: a 24px
    # panel scales it down itself and the result is mush. install.sh copies
    # this tree straight into ~/.local/share/icons/hicolor.
    icons = os.path.join(assets, "hicolor")
    for size, frame in frames.items():
        folder = os.path.join(icons, f"{size}x{size}", "apps")
        os.makedirs(folder, exist_ok=True)
        frame.save(os.path.join(folder, "evesgarden.png"))
    biggest = os.path.join(icons, f"{SIZE}x{SIZE}", "apps")
    os.makedirs(biggest, exist_ok=True)
    master.save(os.path.join(biggest, "evesgarden.png"))

    # Kept so a Windows build from the same source still has its icon.
    ordered = [frames[s] for s in ICO_SIZES]
    ordered[-1].save(os.path.join(assets, "icon.ico"), format="ICO",
...
''')
    ...
    print(f"wrote {os.path.join(assets, 'hicolor')} "
          f"({', '.join(str(s) for s in ICO_SIZES)}, {SIZE})")
