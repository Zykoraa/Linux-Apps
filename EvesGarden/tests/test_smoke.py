...
class SurfaceWalk:
    ...
    def steps(self):
        ...
        # CTk opens an option menu's list as a tkinter.Menu -- a separate
        # X11 override-redirect window. Hyprland ignores where those ask to
        # go and centres them on the monitor, so the list drew ~965px away
        # from the clicks that worked. Drawn inside the toplevel there is no
        # second window to misplace, and this pins that down.
        self.mark("dropdowns open inside the window")
        menu = app.sort_dropdown
        menu._clicked()
        yield 200
        panel = menu._panel
        assert panel is not None, "the sort dropdown did not open"
        top = app.winfo_toplevel()
        assert panel.winfo_toplevel() is top, (
            "the list is a separate window again, which the compositor may "
            "put anywhere")
        left = panel.winfo_rootx() - top.winfo_rootx()
        topy = panel.winfo_rooty() - top.winfo_rooty()
        assert 0 <= left <= top.winfo_width(), (
            "the list sits outside the window horizontally: %d" % left)
        assert 0 <= topy <= top.winfo_height(), (
            "the list sits outside the window vertically: %d" % topy)
        assert abs(panel.winfo_rootx() - menu.winfo_rootx()) < 40, (
            "the list is not under its button: list at %d, button at %d"
            % (panel.winfo_rootx(), menu.winfo_rootx()))

        self.mark("a dropdown closes when clicked again")
        menu._clicked()
        yield 160
        assert menu._panel is None, "clicking the button again left it open"

        self.mark("choosing from a dropdown still fires the command")
        was = app.library_sort
        menu._clicked()
        yield 160
        rows = [w for w in menu._panel_rows]
        assert rows, "the list rendered no rows"
        other = next(r for r in rows if r.cget("text") != was)
        other.invoke()
        yield 260
        assert menu._panel is None, "choosing a value left the list open"
        assert app.library_sort == other.cget("text"), (
            "choosing a value did not reach the command")
        app.set_library_sort(was)
        yield 160

        self.mark("a long list scrolls instead of leaving the window")
        long_list = app.theme_dropdown          # eighteen of them
        long_list._clicked()
        yield 200
        assert long_list._panel is not None, "the theme list did not open"
        bottom = (long_list._panel.winfo_rooty() - top.winfo_rooty()
                  + long_list._panel.winfo_height())
        assert bottom <= top.winfo_height() + 8, (
            "the theme list ran %dpx off the bottom of the window"
            % (bottom - top.winfo_height()))
        long_list.close_dropdown()
        yield 160

        # The visualiser is the backdrop to Now Playing now, sharing that
        # canvas with the blurred cover, the titles and the cards. It draws
        # only tagged items so the rest of the canvas survives, and it is
        # confined to a band along the bottom so it is not a wall of colour
        # with the title somewhere inside it.
        ...
        # Only where a size request is actually granted. A tiling compositor
        # sizes windows itself: geometry() is a suggestion it may ignore and
        # minsize() is a hint it need not honour, so under Hyprland or sway
        # this sweep measured the same tile four times while believing it had
        # resized the window -- and that tile can be narrower than MIN_W, so
        # it then failed on a search box the app was never given room for.
        #
        # Skipped rather than adapted, because the point of the sweep is the
        # widths, and a run that cannot set them is not testing them. CI runs
        # under Xvfb with no window manager at all, where every request is
        # granted, so the coverage is not lost where it does the work.
        app.geometry("1600x700+20+20")
        yield 400
        if abs(app.winfo_width() - 1600) > 40:
            self.mark("header widths: skipped, the window manager owns the size")
            sweep = ()
        else:
            # 900 is the narrowest the window can be dragged to, so it is the
            # one that has to hold.
            sweep = (1600, 1340, 1100, 900)
        ...
        folders = self.gui.dialogs.FolderPicker(
            app, app.theme, [self.gui.LIBRARY_DIR, "/mnt/music/Archive"],
            fixed=(self.gui.LIBRARY_DIR,))
        folders.present()
        yield 300
        # The download folder cannot be removed; the other one can.
        folders._remove("/mnt/music/Archive")
        yield 120
        ...
        # The window must still be one the window manager owns. On Windows
        # the equivalent risk was the opposite way round -- a frameless
        # WS_POPUP that the shell refused to snap until it was told it was
        # sizable -- but here overrideredirect would take the window out of
        # the taskbar and out of Alt-Tab entirely, and the only sign of it
        # is that you cannot get back to the window once it is covered.
        self.mark("window is still managed by the window manager")
        assert not app.wm_overrideredirect(), (
            "the window is override-redirect: no taskbar button, no "
            "Alt-Tab, and no way back to it once something covers it")
        if app.wm.available:
            # WM_STATE is the window manager saying it has adopted this
            # window. Without it the window is on screen but unmanaged.
            assert app.wm._client is not None, "no managed window found"
        yield 80

        # Dragging the title bar hands the move to the window manager with
        # _NET_WM_MOVERESIZE. The Windows original tried the same idea with
        # SendMessage(WM_NCLBUTTONDOWN) and it aborted the interpreter the
        # first time anybody clicked the title bar -- the modal move loop
        # dispatched back into Tk while ctypes had the GIL released:
        #
        #     Fatal Python error: PyEval_RestoreThread: the function must be
        #     called with the GIL held, but the GIL is released
        #
        # A ClientMessage cannot do that, because Python is never inside it.
        # This step passes by the process still being here afterwards, and
        # by the drag falling back to moving the window itself if the window
        # manager declines.
        ...
        # but only where this app is the one doing the snapping. A window
        # manager that accepted the drag did its own snapping already, with
        # its own edge threshold and its own idea of which monitor the
        # pointer is on, and a second snap on top would fight it.
        ...
        app.get_pos(FakeEvent(x=40, y=10, x_root=500, y_root=300))
        yield 60
        wm_took_it = getattr(app, "_wm_dragging", False)
        ...
        if wm_took_it:
            ...
            # Half is narrower than the window is allowed to be, which is the
            # case on a 1024px CI runner: it must decline rather than wedge
            # itself against its own minimum.
