...
class WindowIsManaged(unittest.TestCase):
    """The window must stay one the window manager owns.

    This is the trap the Windows original walks straight into. There,
    overrideredirect(True) is how you draw your own title bar, and the app
    then bought back snapping and tiling by setting WS_THICKFRAME on the
    handle. On X11 the same flag means something much stronger: leave this
    window alone entirely. An override-redirect window has no taskbar
    button, is not in Alt-Tab, belongs to no workspace, is never given
    keyboard focus by the window manager, and under a tiling compositor is
    never placed at all. It is the flag menus and tooltips use.

    The symptom is not a crash. The app looks right, and then it goes behind
    something and there is no way back to it -- which is the kind of thing
    that is easy to reintroduce by copying a line back from the Windows
    version, and impossible to catch in a test that only imports modules.
    So this reads the source.
    """

    LINUX_MODULES = ("gui.py", "dialogs.py", "ui_widgets.py",
                     "linux_window.py", "mpris.py", "recycle.py")
    ...
    def _calls(self, tree):
        ...
        Parsed rather than grepped: the port carries explanations naming the
        calls it removed, and a test that could not tell the difference
    ...
    def test_it_can_tell_a_call_from_a_mention(self):
        """The guards below are only worth having if they are not fooled."""
        described = ast.parse('"""We used to call overrideredirect here."""\n'
                              'x = "overrideredirect"')
        self.assertNotIn("overrideredirect", self._calls(described))
        actually = ast.parse("self.overrideredirect(True)")
        self.assertIn("overrideredirect", self._calls(actually))

    def test_the_main_window_is_not_override_redirect(self):
        called = self._calls(self._tree("gui.py"))
        self.assertNotIn(
            "overrideredirect", called,
            "gui.py calls overrideredirect. On X11 that takes the window out "
            "of the taskbar, out of Alt-Tab and out of the tiling layout, and "
            "leaves no way back to it once it is covered. Undecorate with "
            "_MOTIF_WM_HINTS instead -- see linux_window.")

    def test_no_win32_is_left_anywhere(self):
        """Win32 calls do not fail on Linux -- they are simply never reached.

        ctypes.windll does not exist here, so anything still calling it sits
        inside an except that swallows it and the feature is quietly dead
        rather than loudly broken. Left in, it reads as working code.
        """
        forbidden = ("SendMessageW", "GetWindowLongW", "SetWindowLongW",
                     "SetWindowPos", "RegisterHotKey", "UnregisterHotKey",
                     "SHFileOperationW", "SystemParametersInfoW",
                     "PostThreadMessageW", "GetMessageW")
        for name in self.LINUX_MODULES:
            called = self._calls(self._tree(name))
            for win32 in forbidden:
                self.assertNotIn(
                    win32, called,
                    f"{name} still calls {win32}, which does not exist on "
                    f"this platform.")

    def _literals(self, tree):
        """String literals the code uses, not the ones it talks about.

        Same distinction as above, for the same reason: the port explains
        which Windows path it removed, and a test that could not tell a
        docstring from a filename would fail on the explanation.
        """
        docstrings = set()
        for node in ast.walk(tree):
    ...
    def test_nothing_still_points_at_a_dot_exe(self):
        """A hardcoded ffmpeg.exe is not an error here -- it is silence.

        os.path.join(bin, "ffmpeg.exe") produces a path that does not exist,
        pydub fails to decode, and the app shows "could not decode" for every
        file in the library with nothing to say why.
        """
        for name in ("player_engine.py", "downloader.py", "stream_source.py",
                     "ffmpeg_tools.py", "gui.py"):
            used = self._literals(self._tree(name))
            for windows_name in ("ffmpeg.exe", "ffprobe.exe", "gui.exe"):
                self.assertNotIn(windows_name, used,
                                 f"{name} uses the literal {windows_name}")


class XdgPaths(unittest.TestCase):
    """State goes where the spec says, and the variables are honoured.

    Not pedantry: ~/.config is what people back up and sync between
    machines, ~/.cache is what a disk cleaner may delete without asking.
    Putting the OAuth token cache in the first means a cleanup takes the
    settings with it; putting settings in the second means it takes those.
    """
    ...
    def test_the_four_kinds_of_state_do_not_share_a_directory(self):
        found = {xdg_paths.config_dir(), xdg_paths.data_dir(),
                 xdg_paths.cache_dir(), xdg_paths.state_dir()}
        self.assertEqual(len(found), 4)

    def test_the_environment_wins(self):
        os.environ["XDG_CONFIG_HOME"] = os.path.join(self.tmp, "cfg")
        self.assertEqual(xdg_paths.config_dir(),
                         os.path.join(self.tmp, "cfg", "evesgarden"))

    def test_a_relative_value_is_ignored(self):
        """The spec says so, and an empty XDG_CONFIG_HOME= is common.

        Honouring one would put the config in the process working directory,
        which for a desktop launch is /.
        """
        os.environ["XDG_CONFIG_HOME"] = "relative/path"
        self.assertTrue(os.path.isabs(xdg_paths.config_dir()))
        os.environ["XDG_CONFIG_HOME"] = ""
        self.assertTrue(os.path.isabs(xdg_paths.config_dir()))

    def test_the_music_folder_is_the_one_the_desktop_configured(self):
        """A hardcoded ~/Music is wrong on a non-English install.

        xdg-user-dirs writes the real name, which is ~/Musique or ~/Musik
        there, and creating ~/Music beside it would give the user a second
        music folder they never asked for.
        """
        os.environ["XDG_CONFIG_HOME"] = self.tmp
        with open(os.path.join(self.tmp, "user-dirs.dirs"), "w") as handle:
            handle.write('XDG_MUSIC_DIR="$HOME/Musique"\n')
        found = xdg_paths._xdg_user_dir("MUSIC")
        # Only meaningful when xdg-user-dir is absent; when it is installed
        # it answers from the real session, which is the right answer too.
        if found is not None and "Musique" not in found:
            self.assertTrue(os.path.isabs(found))
        else:
            self.assertEqual(found, os.path.expanduser("~/Musique"))


class Trash(unittest.TestCase):
    """Deleting music must be undoable, and it must not cross a filesystem.

    The Windows build sent files to the Recycle Bin and fell back to
    os.remove when the shell refused. That fallback ran on every non-Windows
    machine, so the Linux port of it as written would have turned "move 300
    duplicates to the trash" into "erase 300 files".
    """
    ...
    def _trash_root(self):
        return os.path.join(self.tmp, "data", "Trash")

    def _read(self, path):
        with open(path, encoding="utf-8") as handle:
            return handle.read()

    def test_a_trashed_file_leaves_the_library_and_lands_in_the_trash(self):
        path = self._make("song.mp3")
        trashed, failed = recycle.send_to_recycle_bin([path])
        self.assertEqual((trashed, failed), ([path], []))
        self.assertFalse(os.path.exists(path))
        self.assertTrue(os.path.exists(
            os.path.join(self._trash_root(), "files", "song.mp3")))

    def test_it_records_where_the_file_came_from(self):
        """Without a .trashinfo there is nowhere to restore to.

        A file manager shows an entry with no origin as an orphan and offers
        no Restore, which makes the whole point of trashing rather than
        deleting -- that it is undoable -- not true.
        """
        path = self._make("song.mp3")
        recycle.send_to_recycle_bin([path])
        info = os.path.join(self._trash_root(), "info", "song.mp3.trashinfo")
        with open(info, encoding="utf-8") as handle:
            body = handle.read()
        self.assertIn("[Trash Info]", body)
        self.assertIn(f"Path={path}", body)
        self.assertRegex(body, r"DeletionDate=\d{4}-\d\d-\d\dT\d\d:\d\d:\d\d")

    def test_two_files_with_one_name_do_not_overwrite_each_other(self):
        """Duplicates are exactly what this feature deletes, so they collide."""
        first = self._make("song.mp3", "first")
        second = self._make(os.path.join("other", "song.mp3"), "second")
        trashed, failed = recycle.send_to_recycle_bin([first, second])
        self.assertEqual(len(trashed), 2)
        self.assertEqual(failed, [])
        files = sorted(os.listdir(os.path.join(self._trash_root(), "files")))
        self.assertEqual(len(files), 2)
        bodies = sorted(self._read(os.path.join(self._trash_root(), "files", f))
                        for f in files)
        self.assertEqual(bodies, ["first", "second"])

    def test_a_file_that_cannot_be_trashed_is_reported_not_deleted(self):
        path = self._make("song.mp3")
        os.chmod(self.music, 0o555)          # cannot rename out of it
        try:
            trashed, failed = recycle.send_to_recycle_bin([path])
        finally:
            os.chmod(self.music, 0o755)
        if failed:                            # root can rename regardless
            self.assertEqual(trashed, [])
            self.assertTrue(os.path.exists(path),
                            "a file that could not be trashed was deleted")

    def test_nothing_to_do_is_not_an_error(self):
        self.assertEqual(recycle.send_to_recycle_bin([]), ([], []))
        self.assertEqual(recycle.send_to_recycle_bin([None, "/nonexistent"]),
                         ([], []))


class Ffmpeg(unittest.TestCase):
    """The system copy is preferred; a bundled one is only a fallback.

    The other way round -- which is what the Windows build hardcoded -- ships
    a second ffmpeg that never gets a security update, on a platform where
    the packaged one is already installed and patched.
    """

    def test_it_finds_the_system_ffmpeg(self):
        found = ffmpeg_tools.ffmpeg()
        if found is None:
            self.skipTest("no ffmpeg installed")
        self.assertTrue(os.path.isabs(found))
        self.assertTrue(os.access(found, os.X_OK))

    def test_a_missing_ffmpeg_says_how_to_install_one(self):
        self.assertIn("pacman", ffmpeg_tools.MISSING_MESSAGE)
        self.assertIn("apt", ffmpeg_tools.MISSING_MESSAGE)

    def test_yt_dlp_is_given_a_directory_not_a_binary(self):
        """yt-dlp takes ffmpeg_location as a folder; a path to the binary
        makes it silently fall back to whatever is on PATH."""
        path = downloader.get_ffmpeg_path()
        self.assertTrue(os.path.isdir(path) or not os.path.exists(path))
        self.assertNotIn("ffmpeg", os.path.basename(path))


class MprisMetadata(unittest.TestCase):
    """What the panel is told. No bus needed -- this is the mapping only."""

    def setUp(self):
        try:
            self.Player = mpris._Lazy.get()[1]
        except ImportError:
            self.skipTest("dbus-fast not installed")

    def _player(self):
        return self.Player.__new__(self.Player)

    def _build(self, **payload):
        base = dict(state="Playing", title="A Title", artist="An Artist",
                    album="An Album", duration=210.0, art_url=None,
                    track_path="/org/mpris/MediaPlayer2/Track/1")
        base.update(payload)
        return self.Player._build_metadata(self._player(), base)

    def test_the_artist_is_a_list(self):
        """xesam:artist is `as`. A bare string marshals as one-character
        entries, and the panel shows the first letter of the name."""
        data = self._build()
        self.assertEqual(data["xesam:artist"].signature, "as")
        self.assertEqual(data["xesam:artist"].value, ["An Artist"])

    def test_the_length_is_microseconds(self):
        self.assertEqual(self._build()["mpris:length"].value, 210_000_000)

    def test_a_track_of_unknown_length_omits_the_length(self):
        """Zero would draw a scrubber that is permanently at the end."""
        self.assertNotIn("mpris:length", self._build(duration=0.0))

    def test_stopped_with_nothing_playing_is_the_no_track_path(self):
        data = self._build(state="Stopped", title=None)
        self.assertEqual(data["mpris:trackid"].value, mpris.NO_TRACK)
        self.assertNotIn("xesam:title", data)

    def test_the_track_id_is_an_object_path(self):
        """`o` is validated on the wire: a bad path drops the whole update."""
        self.assertEqual(self._build()["mpris:trackid"].signature, "o")
        self.assertTrue(self._build()["mpris:trackid"].value.startswith("/"))

    def test_artwork_is_a_url_with_spaces_escaped(self):
        url = mpris.file_url("/home/eve/Music/Eve's Garden/a b.jpg")
        self.assertTrue(url.startswith("file:///"))
        self.assertNotIn(" ", url)


class RevealInFileManager(unittest.TestCase):
    """`explorer /select,` has no single equivalent, so this degrades."""

    def test_a_path_that_is_not_there_is_declined_not_attempted(self):
        self.assertFalse(file_manager.reveal("/nonexistent/nothing.mp3"))

    def test_the_uri_is_escaped(self):
        """The library folder is "Eve's Garden" -- a space and an apostrophe.

        Unescaped, the space ends the URI and the file manager opens the
        wrong folder or nothing at all.
        """
        from urllib.parse import quote
        uri = "file://" + quote("/home/x/Music/Eve's Garden/a b.mp3")
        self.assertNotIn(" ", uri)
        self.assertTrue(uri.startswith("file:///home/x/Music/"))

    def test_opening_a_folder_that_is_not_one_is_declined(self):
        self.assertFalse(file_manager._open_folder("/nonexistent"))
        self.assertFalse(file_manager._open_folder(""))


class CoverCache(unittest.TestCase):
    """MPRIS artwork has to be a file, because the property is a URL."""
    ...
    def test_a_file_with_no_art_is_not_an_error(self):
        path = os.path.join(self.tmp, "silent.mp3")
        with open(path, "wb") as handle:
            handle.write(b"not audio")
        self.assertIsNone(audio_files.cover_file(path, cache_dir=self.tmp))

    def test_a_missing_file_is_not_an_error(self):
        self.assertIsNone(audio_files.cover_file("/nonexistent.mp3",
                                                 cache_dir=self.tmp))
        self.assertIsNone(audio_files.cover_file(None, cache_dir=self.tmp))


def _ffmpeg():
    """ffmpeg, if this machine has one. A bare CI runner does not."""
