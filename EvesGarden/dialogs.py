...
The app deliberately sheds system chrome: the main window draws its own title
bar. The dialogs never got that treatment. Asking for a playlist name opened
a stock CTkInputDialog and the artist album picker a bare CTkToplevel, so
both arrived with a system frame, a system title bar, square corners and a
different close button -- sitting on top of an app that had gone to some
trouble not to have any of that.
...
They are frameless the same way the main window is, too: undecorated but
still managed, rather than override-redirect. On Windows that distinction did
not matter for a dialog. On X11 it decides whether you can type into one.
The window manager hands out keyboard focus, and it does not hand it to a
window it was told to ignore -- so the name field in TextPrompt had the Tk
focus, showed a caret, and received nothing, because the key events were
still going to the main window. Making the dialog managed and transient is
what puts the caret and the keystrokes in the same window.
"""
...
class ModalDialog(ctk.CTkToplevel):
    ...
    def __init__(self, parent, theme, title, size=None, body_pad=(24, 20)):
        ...
        # Hidden until _appear: the Motif hints have to be on the window
        # before the window manager maps it, or a desktop that decorates
        # would draw a title bar and take it away again a frame later.
        # withdraw() leaves it unmapped; update_idletasks() still creates the
        # X window, which is all that setting a property needs.
        self.withdraw()
        self.update_idletasks()
        self.wm = linux_window.LinuxWindow(self)
        self.wm.undecorate()

        # transient(), not -topmost. Both keep the dialog above the app, but
        # -topmost is _NET_WM_STATE_ABOVE, which puts it above every other
        # application on the desktop as well -- so alt-tabbing to a browser
        # left the dialog floating over it. transient() says "this belongs to
        # that window", which is what stacks it correctly, gives it the
        # dialog treatment on every desktop, and moves it with its parent.
    ...
    def _drag(self, event):
        # Same handover as the main window: the window manager takes the
        # pointer when it accepts the drag, so a motion event still arriving
        # means it declined and we move the dialog ourselves.
    ...
    def close(self, result=None):
        ...
        def finished():
            # The X connection is the dialog's own, and dialogs are opened
            # and closed repeatedly -- a picker per artist. Leaking one
            # socket per dialog runs the process out of file descriptors.
            try:
                self.wm.close()
            except Exception:
                pass
