...
# ------------------------------------------------- art, as a file on disk

# Cover art is embedded in the audio file, which is fine for anything that
# wants bytes. MPRIS does not: the artwork is a URL, so a panel can read the
# image itself without the player sending it. That means the art has to exist
# as a file, which is what this writes -- once per track, into the cache,
# where a disk cleaner is welcome to delete it.
COVER_CACHE_LIMIT = 400

_MIME_EXTENSIONS = {b"\xff\xd8\xff": ".jpg", b"\x89PNG": ".png"}


def _cover_extension(data):
    for magic, ext in _MIME_EXTENSIONS.items():
        if data.startswith(magic):
            return ext
    return ".img"


def cover_file(path, cache_dir=None):
    """Embedded cover art written out as a file. Returns its path, or None.

    Keyed on size and mtime as well as the path, so re-tagging a file with
    new artwork produces a new cache entry rather than serving the old
    picture until someone clears the cache by hand.
    """
    import hashlib
    ...
    if cache_dir is None:
        import xdg_paths
        cache_dir = os.path.join(xdg_paths.cache_dir(), "covers")

    key = f"{os.path.abspath(path)}:{stat.st_size}:{int(stat.st_mtime)}"
    digest = hashlib.sha1(key.encode("utf-8", "replace")).hexdigest()

    try:
        existing = [f for f in os.listdir(cache_dir) if f.startswith(digest)]
        if existing:
    ...
    target = os.path.join(cache_dir, digest + _cover_extension(data))
    try:
        os.makedirs(cache_dir, exist_ok=True)
        # Written under a temporary name and renamed, so a panel that reads
        # the file the moment it appears never gets half an image.
        temp = target + f".{os.getpid()}.part"
...
def _prune_cover_cache(cache_dir):
    """Keep the newest COVER_CACHE_LIMIT covers and drop the rest."""
    try:
        names = os.listdir(cache_dir)
        if len(names) <= COVER_CACHE_LIMIT:
            return
        entries = []
        for name in names:
        ...
        for _, full in entries[COVER_CACHE_LIMIT:]:
            try:
                os.remove(full)
            except OSError:
                pass
