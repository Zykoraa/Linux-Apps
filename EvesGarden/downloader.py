...
def get_config_dir():
    """Where the Spotify token cache goes.

    ~/.cache, not ~/.config. It is a refreshable OAuth token, not a setting:
    deleting it costs one sign-in, and it has no business in the directory
    people back up and sync between machines.
    """
    return xdg_paths.ensure(xdg_paths.cache_dir())
...
def get_ffmpeg_path():
    """The directory yt-dlp should look in for ffmpeg.

    yt-dlp takes a directory, not a binary, so this hands it whichever one
    the ffmpeg we resolved actually lives in -- /usr/bin for a system
    install, the bundled bin/ for a self-contained build.
    """
...
def _ffmpeg_exe():
    return ffmpeg_tools.ffmpeg() or "ffmpeg"
...
def sanitize_filename(filename):
    """Make a string safe to use as a filename anywhere.

    Linux forbids only "/" and NUL, so most of this is stricter than it has
    to be -- deliberately. Music gets copied to a phone, a USB stick or a
    network share, and a file called `AC/DC - Back In Black.mp3` that a
    Linux-only rule let through would fail to copy to any of them. Keeping
    the Windows rules also keeps the "already downloaded?" check agreeing
    with a library that was downloaded on Windows.
    """
