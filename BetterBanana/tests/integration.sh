...
# Quote every use of these below: the repository can live under a path with a
# space in it, and an unquoted "$CTL status" then word-splits into a command
# that does not exist - which this script reported as "engine not running".
...
# Each parked stream with the NAME of the sink it was on, so it can be put back
# exactly where it came from. Restoring them all to bb_vaio instead - as this
# did until 2026-09-02 - silently re-routes an application that was on a cable,
# and a strip fed by a cable may be the only thing feeding a bus. The symptom is
# an application that has gone quiet for no visible reason after a test run.
streams_on_ours_named () {
  local map ids
  map=$(pactl list short sinks | awk '{ print $1 "=" $2 }')
  ids=$(pactl list short sinks | awk '$2 ~ /^bb_/ { print $1 }' | tr '\n' '|' | sed 's/|$//')
  [ -z "$ids" ] && return 0
  pactl list short sink-inputs | awk -v re="^($ids)$" -v m="$map" '
    BEGIN { n = split(m, rows, "\n");
            for (i = 1; i <= n; i++) { split(rows[i], kv, "="); name[kv[1]] = kv[2] } }
    $2 ~ re { print $1 " " name[$2] }'
}

PARKED="$(streams_on_ours_named)"
park () {
  [ -z "$FALLBACK" ] && return 0
  printf '%s\n' "$PARKED" | while read -r i _; do
    [ -n "$i" ] && pactl move-sink-input "$i" "$FALLBACK" 2>/dev/null
  done
}
restore_all () {
  printf '%s\n' "$PARKED" | while read -r i sink; do
    [ -n "$i" ] && [ -n "$sink" ] && pactl move-sink-input "$i" "$sink" 2>/dev/null
  done
...
if [ -n "$(printf '%s' "$PARKED" | tr -d '[:space:]')" ]; then
  echo "  (parking $(printf '%s\n' "$PARKED" | grep -c . ) stream(s) off the virtual sinks; they go back where they were)"
  park
  sleep 0.5
fi
